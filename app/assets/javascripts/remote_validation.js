        ...
        showErrors: function (form, settings, errors) {
            ...
            for (var col in errors) {
                for (var i in errors[col]) {
                    ...
                    $("*[name='" + formFor + "[" + col + '(1i)' + "]']", form).wrap($("<div/>").addClass(settings.errorClass));
                    $("*[name='" + formFor + "[" + col + '(2i)' + "]']", form).wrap($("<div/>").addClass(settings.errorClass));
                    $("*[name='" + formFor + "[" + col + '(3i)' + "]']", form).wrap($("<div/>").addClass(settings.errorClass));
            ...
            $this.on('ajax:beforeSend',function (evt, xhr) {
                ...
                $("." + settings.errorClass, $this).children().unwrap();
                    ...
                    if (data.errors) {
                        ...
                        if ($('title').html() == 'Step 1') {
                            $(this).hide();
                            $('title').html('Step 2');
                            $("#step_2").show();
                        }
                        else if ($('title').html() == 'Step 2') {
                            $(this).hide();
                            $('title').html('Step 3');
                            $("#step_3").show();
                            $('#countdown').countdown('pause');
                            window.localStorage.amount = $('.countdown_amount').text();
